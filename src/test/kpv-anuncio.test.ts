...
describe('texto para a interface', () => {
  ...
  it('explica a confiança em vez de só rotular', () => {
    ...
    expect(explicarConfianca('alta', 20)).toMatch(/mais de uma fonte/);
    expect(explicarConfianca('media', 1)).toMatch(/1 anúncio\b/);
    expect(explicarConfianca('media', 8)).toMatch(/8 anúncios/);
  });
});
