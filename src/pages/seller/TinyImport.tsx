// Importar catálogo do Tiny (Olist ERP).
//
// Espelha `BlingImport.tsx`: mesma ideia, fonte diferente. O princípio é o
// mesmo, e é o que importa: CONFERIR antes de criar. O lojista descobre na tela
// o que falta (foto, escala) antes de criar 200 anúncios travados na análise.
//
// O Tiny não tem categoria da Kolecta nem condição do item, então esses dois
...
import {
  useTinyStatus, useTinyProdutos, useTinyConferir, useTinyImportar,
} from '@/hooks/use-api';
import { CATEGORY_FIELDS } from '@/lib/category-fields';
import { CONDITIONS } from '@/lib/conditions';
import { formatBRL } from '@/lib/currency';

/** Teto por lote, igual ao do backend (importar baixa e recopia cada foto). */
const MAX_LOTE = 15;

export default function TinyImportPage() {
  const { data: status, isLoading: carregandoStatus } = useTinyStatus();
  const conectado = !!status?.connected && !status?.expired;

  const [pagina, setPagina] = useState(1);
  // Busca por nome. Debounced pra não bater no Tiny a cada tecla, e volta pra
  // página 1 quando o termo muda (senão o vendedor buscaria e cairia numa
  // página vazia do resultado antigo). Retorno de founder: catálogo grande
  // obrigava paginar tudo.
  ...
  const { data: catalogo, isLoading: carregandoCatalogo } = useTinyProdutos(pagina, conectado, buscaDeb);
  ...
  const conferir = useTinyConferir();
  const importar = useTinyImportar();

  const produtos = catalogo?.produtos ?? [];
  const podeConferir = marcados.length > 0 && !!categoria && !!condicao;

  // A conferência vale só para o lote conferido. Mudou a seleção ou as escolhas,
  // o resultado velho engana: some com ele.
        ...
        <div className="max-w-2xl space-y-4">
          ...
          <Card className="bg-gradient-card">
            <CardContent className="p-6 space-y-3">
              ...
              <p className="font-heading text-lg font-bold">Conecte seu Tiny primeiro</p>
  ...
  return (
    <SellerLayout>
      <div className="max-w-4xl space-y-5">
        <Voltar />
        <div>
          <h1 className="font-heading text-3xl font-bold">Importar do Tiny</h1>
        ...
        <Card className="bg-gradient-card">
          ...
          <CardContent className="space-y-4">
            <p className="text-xs text-muted-foreground">
              O Tiny não guarda categoria nem condição, então esses dois você
            ...
            {camposDoLote.length > 0 && (
              <div className="space-y-3 border-t border-border pt-4">
                <p className="text-xs text-muted-foreground">
                  O Tiny não tem estes campos e a categoria exige. Preenchem o
          ...
          <CardContent className="space-y-3">
            ...
            {carregandoCatalogo ? (
              ...
              <p className="text-sm text-muted-foreground py-6 text-center">
                Nenhum produto nesta página do seu Tiny.
              </p>
            ) : (
              produtos.map((p) => {
                const marcado = marcados.includes(p.id);
                const cheio = marcados.length >= MAX_LOTE && !marcado;
                const item = conferencia?.itens.find((i) => i.tinyProductId === p.id);
                return (
                  <label
                    key={p.id}
