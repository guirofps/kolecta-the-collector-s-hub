// ─── Ícone da categoria ──────────────────────────────────────────────────────
// Este switch estava copiado igual em cinco arquivos (home, categorias, página
// da categoria, perfil do vendedor e wizard de anúncio). Ao criar a categoria
// de Acessórios seria a sexta cópia, e qualquer categoria nova depois dela
// nasceria faltando ícone em algum canto. Aqui fica a única versão.
...
export function CategoryIcon({ slug, size = 32 }: { slug: string; size?: number }) {
  ...
  switch (slug) {
    ...
    case 'cards-colecionaveis':
      return (
        <svg width={size} height={size} viewBox="0 0 32 32" fill="none" aria-hidden="true">
          ...
          <polygon
            points="16,9 17.8,14 23,14 18.9,17 20.5,22 16,19 11.5,22 13.1,17 9,14 14.2,14"
            fill="#0f0f0f"
            opacity="0.3"
          />
        </svg>
    ...
    // Roda: é o acessório mais vendido hoje e o desenho mais reconhecível para
    // "peça de miniatura" (expositor e decal não têm silhueta própria).
    case 'acessorios':
      return (
        <svg width={size} height={size} viewBox="0 0 32 32" fill="none" aria-hidden="true">
          <circle cx="16" cy="16" r="13" fill={fill} />
          <circle cx="16" cy="16" r="4.5" fill="#0f0f0f" opacity="0.35" />
          <path
            d="M16 4 L16 11 M16 21 L16 28 M4 16 L11 16 M21 16 L28 16"
            stroke="#0f0f0f"
            strokeWidth="2.2"
            strokeLinecap="round"
            opacity="0.35"
