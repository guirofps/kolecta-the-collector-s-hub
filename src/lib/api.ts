...
export const api = {
  ...
  bling: {
    ...
    /** Uma página do catálogo do Bling do lojista. `busca` filtra por nome no ERP. */
    produtos: (token: string, pagina: number, busca?: string) =>
      request<{ data: BlingCatalogoPagina }>(
        `/api/bling/produtos?pagina=${pagina}${busca?.trim() ? `&busca=${encodeURIComponent(busca.trim())}` : ''}`,
        { token },
      ).then(r => r.data),
